(function(angular, factory) {
    ...
    app.factory('NgTableParams', ['$q', '$log', 'ngTableDefaults', function($q, $log, ngTableDefaults) {
        ...
        var NgTableParams = function(baseParameters, baseSettings) {
            ...
            /**
             * turn to page number directly when enter key down
             * @param event
             */
            this.gotoPage = function(event){
                if(event.keyCode==13){
                    var totalPage = Math.ceil(this.total() / this.count());
                    this.toPageNum = this.toPageNum>0?(this.toPageNum > totalPage?totalPage:this.toPageNum):1;
                    this.page(this.toPageNum);
                }
            };
    ...
    angular.module('ngTable').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('ng-table/header.html', '<tr class="table-title"><th title="{{$column.headerTitle(this)}}" ng-repeat="$column in $columns" ng-class="{ \'sortable\': $column.sortable(this), \'sort-asc\': params.sorting()[$column.sortable(this)]==\'asc\', \'sort-desc\': params.sorting()[$column.sortable(this)]==\'desc\' }" ng-click="sortBy($column, $event)" ng-show="$column.show(this)" ng-init="template=$column.headerTemplateURL(this)" class="{{$column.class(this)}}"> <div ng-if="!template" ng-show="!template" class="" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'div\'}"> <span ng-bind="$column.title(this)" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'span\'}"></span> </div> <div ng-if="template" ng-show="template" ng-include="template"></div> </th> </tr> <tr ng-show="show_filter" class="ng-table-filters"> <th data-title-text="{{$column.titleAlt(this) || $column.title(this)}}" ng-repeat="$column in $columns" ng-show="$column.show(this)" class="filter"> <div ng-repeat="(name, filter) in $column.filter(this)"> <div ng-if="filter.indexOf(\'/\') !==-1" ng-include="filter"></div> <div ng-if="filter.indexOf(\'/\')===-1" ng-include="\'ng-table/filters/\' + filter + \'.html\'"></div> </div> </th> </tr> ');
        $templateCache.put('ng-table/pager.html', '<div class="ng-cloak ng-table-pager" ng-if="params.data.length"> <div ng-if="params.settings().counts.length" class="jumppage-left clearfix"><span>共 <b>{{params.total()}}</b> 条，每页显示</span> <button ng-repeat="count in params.settings().counts" type="button" ng-class="{\'active\':params.count()==count}" ng-click="params.count(count)" class="btn btn-default"> <span ng-bind="count"></span> </button> </div>  <div class="jumppage-right"><ul class="clearfix"> <li></li><li ng-class="{\'disabled\': !page.active && !page.current, \'current\': page.current}" ng-repeat="page in pages" ng-switch="page.type"><a ng-switch-when="prev" ng-click="params.page(page.number)" href="">&laquo;</a> <a ng-switch-when="first" ng-click="params.page(page.number)" ng-class="{\'disabled\': !page.active && !page.current, \'current\': page.current}" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="page" ng-click="params.page(page.number)" ng-class="{\'disabled\': !page.active && !page.current, \'current\': page.current}" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a> <a ng-switch-when="last" ng-click="params.page(page.number)" ng-class="{\'disabled\': !page.active && !page.current, \'current\': page.current}" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="next" ng-click="params.page(page.number)" href="">&raquo;</a> </li></ul></div> </div> ');
    }]);
    return app;
}));